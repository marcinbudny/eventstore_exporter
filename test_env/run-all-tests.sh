...
docker-compose -f 22.6-single/docker-compose.yml down
...
echo "Running tests on ES 22.6 - SINGLE"
EVENTSTORE_RUN_PROJECTIONS=None docker-compose -f 22.6-single/docker-compose.yml up -d
sleep 5
TEST_EVENTSTORE_URL=http://localhost:2113 go test ../... -count=1 -v
docker-compose -f 22.6-single/docker-compose.yml down

echo "Running tests on ES 22.6 - SINGLE - WITH PROJECTIONS"
EVENTSTORE_RUN_PROJECTIONS=All docker-compose -f 22.6-single/docker-compose.yml up -d
sleep 5
TEST_EVENTSTORE_URL=http://localhost:2113 TEST_PROJECTION_METRICS=1 go test ../... -count=1 -v
docker-compose -f 22.6-single/docker-compose.yml down